...
public class RedisService {
    ...
    public <T> T hget(String key, String item) {
        try {
            if (key == null)
                return null;
            T result;
            result = (T) redisTemplate.opsForHash().get(key, item);
            if (result != null) {
                return result;
            }
            return null;
    ...
    public <T> List<T> hgetList(String key, String item) {
        try {
            if (key == null)
                return null;
            List<T> result;
            result = (List<T>) redisTemplate.opsForHash().get(key, item);
            if (result != null) {
                return result;
            }
            return null;
    ...
    public <T> List<T> hmvget(String key, Class<T> c) {
        try {
            ...
            result = (List<T>) redisTemplate.opsForHash().values(key);
            return result;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
     ...
     * @return 对应的多个键值
     */
    public <T> Map<Object, Object> hmget(String key) {
        try {
    ...
    public <T> List<T> lGet(String key, long start, long end, final Class<T> c) {
        try {
            ...
            List<T> result;
            result = (List<T>) redisTemplate.opsForList().range(key, start, end);
            if (result.size() == 0) {
                return null;
            }
            return result;
